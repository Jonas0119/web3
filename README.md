...
### 2025-09-19 TabBar 字体大小优化

#### 修改原因
底部导航 Tab 文字在部分设备上显示偏小，影响可读性与可点击性。

#### 修改思路
1. 统一字体族：为 `CustomTabBar.vue` 的 `.custom-tab-bar, .tab-text` 设置与首页一致的 `font-family`。
2. 删除图标：移除 TabBar 图标模板与数据，仅保留文字标签，避免无效资源与空白占位。
3. 调整样式：
   - `.tab-text` 字号设为 `36rpx`；
   - 收紧整体高度至 `110rpx`，去除为图标预留的间距；
   - 保留激活态变色与字重增强。

#### 影响范围
- H5 / 小程序 / App 各端底部导航文案统一字体并放大，更清晰易读；
- 移除无展示价值的图标，减少无效渲染与资源引用。

#### 修改文件
- `components/TabBar/CustomTabBar.vue`

#### 验证结果
- ✅ Tab 文案字号调整为 36rpx，无布局抖动；
- ✅ 字体与首页一致；
- ✅ 图标相关代码与样式已移除；
